...
- Project-facing `lock`, `recheck`, and audit wording was replaced or recast in journal language in the active main and supplementary text. Numerical checks, validation limits, equations, values, citations, parameters, qualified claims, and source curves were retained.
- Twelve active vector figures (the manuscript excludes Figure 4) were rendered and inspected. Twenty-four legend frames across Figures 1–2 and 5–14 were moved below their axes; Figure 3's explanatory key remains in clear diagram whitespace. The FEM_3-styled `figures_gen/` sources use the same external placement. Plotted curves and markers were not redrawn; the JSON/YAML inputs are not present in the package.
- No local TeX engine is installed. The branch-specific GitHub Actions workflow is the required build path. The final-size PDF review and page-by-page checklist will be completed after the workflow rebuild; update this addendum with its run, final page count, and audit result.


> **Later first-pass Introduction update — 2026-09-26:** After the cleanup report was produced, the newer Paper 9 source received a table-free Introduction rewrite and its bibliography was expanded from 15 to 50 entries (50 unique Introduction citations). The earlier bibliography-identity statements below describe the historical pre-revision package and are superseded for `OVERLEAF/sections/sec01_intro.tex` and `OVERLEAF/references.bib`. The current review PDF is `preview/Introduction_Review_Preview.pdf`; it is a ReportLab editorial proof, not a native TeX build. See `INTRODUCTION_REVISION_NOTES.md` for the authoritative Phase 1 scope, checks, and build limitation. This was the status at that time; the full first-pass language edit has since covered Sections 1–9, both appendices, and the supplementary material, followed by the current second-pass refinements summarized above.
