...
This appendix derives the high-wavenumber phase-speed limit for the one-dimensional transverse-shear branch of the stated strain-gradient model. Without micro-inertia ($\ell_{\mathrm{i}}=0$), the branch has unbounded phase velocity as $k\to\infty$; with micro-inertia ($\ell_{\mathrm{i}}>0$), its phase velocity approaches a finite asymptote.
...
\end{equation}
Thus, within this one-dimensional transverse-shear model, omitting micro-inertia gives unbounded high-wavenumber phase and group velocities.
...
\end{equation}
For our baseline Case H parameters ($l_{\mathrm{eff}} = l_{\mathrm{iso}} = 0.20\,\mathrm{m}$, $\ell_{\mathrm{i}} = 0.20\,\mathrm{m}$ from Table~\ref{tab:case_params}, with $\bar{l}_{\mathrm{eff}} = 0.20$ and $\bar{\ell}_{\mathrm{i}} = 0.20$), the transverse asymptotic horizon is:
\begin{equation}
\bar{v}_{T,\infty} = \frac{0.20}{\sqrt{10} \times 0.20} = \frac{1}{\sqrt{10}} \approx 0.3162.
\label{eq:appA_horizon_val}
\end{equation}
As verified numerically in Section~\ref{sec:steering}, the finite element phase velocity at $\bar{k} = 200$ attains $\bar{v}_p = 0.3163$, matching the analytical horizon within $0.03\%$. For this transverse branch, micro-inertia gives a finite high-wavenumber phase-speed limit.
