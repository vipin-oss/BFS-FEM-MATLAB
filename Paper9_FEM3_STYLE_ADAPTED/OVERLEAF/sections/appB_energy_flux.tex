...
\end{equation}
which connects the group velocity $\bm{v}_g = \nabla_{\bmk}\omega$ to the cell-averaged energy flux and stored energy. The source manuscript records a prior group-velocity/energy-flux residual of $6.91\times10^{-10}$ (Table~\ref{tab:consistency_suite}); after correcting the harmonic micro-inertial sign in Eq.~\eqref{eq:appB_S_cycle}, that numerical check must be rerun before it is treated as verified. The raw harmonic fields needed to recompute it are not included in the source package.
