...
The internal suite covers operator Hermiticity, Brillouin-zone periodicity, isotropic rotational invariance, acoustic slopes, rotation--swap symmetry, matrix definiteness, the micro-inertia limit, and the energy-flux/group-velocity identity. Table~\ref{tab:consistency_suite} lists the previously reported values. We have marked the energy-flux residual for recheck after correcting the sign of the harmonic micro-inertia term in Appendix~\ref{app:energy_flux}; the raw fields needed to rerun it are unavailable, so it is not counted as a current pass. Supplementary Material~S3 gives the check definitions and expanded diagnostics.
