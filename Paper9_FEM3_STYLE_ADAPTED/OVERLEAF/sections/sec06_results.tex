...
We proceed from the homogeneous isotropic reference to tensor-driven branch shifts, aspect-ratio and orientation effects, directional stop bands and their sampled design map, and group-velocity steering. We then treat the heterogeneous Case~C separately because it introduces periodic material contrast. Table~\ref{tab:case_params} lists selected inputs; Supplementary Material~S1 gives the full parameter set.
...
Figure~\ref{fig:ar_sweep} isolates elongation at fixed $\theta=45^\circ$, using volume-equivalent length scaling. As $\mathrm{AR}$ increases, the branches shift relative to one another and the directional gap varies non-monotonically; elongation does not produce a monotone increase in the sampled zone-edge frequencies.
