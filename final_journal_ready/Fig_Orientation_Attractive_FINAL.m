%% Fig_Orientation_Attractive_FINAL.m
%  Attractive 4-panel figure with ellipsoid neighbourhood (a)
%  and orientation sweep (b)-(d) - monotonic response.
%
%  Parameters: Lx=1.0, Ly=Lz=0.2, E=200e9, nu=0.30, rho=7800
%  lx=0.20, ly=lz=0.02, rotation about z, 4x2x2 tricubic Hermite
%  Fixed mass matrix: Mscaled = Sscale*Mff*Sscale, eigs 6 modes
...
nelx = 4; nely = 2; nelz = 2;
traction_right = [1.0e6; 0.0; 0.0];
body_force     = [0.0; 0.0; 0.0];
bc_case        = "clamped_gradient";

lx = 0.20; ly = 0.02; lz = 0.02;
theta_list = [0, 15, 30, 45, 60, 75, 90];
nTheta = numel(theta_list);

u_all   = zeros(1, nTheta);
f1_all  = zeros(1, nTheta);
cond_all = zeros(1, nTheta);

fprintf('--- Orientation sweep lx=%.2f ly=lz=%.2f ---\n', lx, ly);
for jt = 1:nTheta
    th = theta_list(jt);
    AAT = build_rotated_AAT(lx, ly, lz, th);
    [u_all(jt), ~, f1_all(jt), cond_all(jt)] = solve_3d_sg_rotated(...
        Lx,Ly,Lz,E,nu,rho,nelx,nely,nelz,AAT,traction_right,body_force,bc_case);
    fprintf('theta=%3d  u=%.4f um  f1=%.2f Hz  kappa=%.3e\n', th, u_all(jt)*1e6, f1_all(jt), cond_all(jt));
end

l_iso = (lx*ly*lz)^(1/3);
AAT_iso = diag([l_iso^2, l_iso^2, l_iso^2]);
[~,~,f_iso,~] = solve_3d_sg_rotated(Lx,Ly,Lz,E,nu,rho,nelx,nely,nelz,AAT_iso,traction_right,body_force,bc_case);
u0 = u_all(1); f0 = f1_all(1);

fprintf('\nIsotropic l_iso=%.5f f_iso=%.2f Hz\n', l_iso, f_iso);
fprintf('u0=%.6e f0=%.2f\n', u0, f0);

u_ratio0 = u_all / u0;
f_ratio_iso = f1_all / f_iso;
f_ratio0 = f1_all / f0;

%% Figure - 2x2 attractive
fig = figure('Color','white','Units','centimeters','Position',[2 2 18 14]);

tl = tiledlayout(2,2,'TileSpacing','compact','Padding','compact');

% Panel (a) - ellipsoids at 0,45,90 - improved view
ax1 = nexttile;
hold(ax1,'on');
% colors matching old attractive fig
col0 = [0.0 0.447 0.741]; % blue
col45 = [0.466 0.674 0.188]; % green
col90 = [0.635 0.078 0.184]; % red

plot_ellipsoid(ax1, lx, ly, lz, 0, col0, 0.7);
plot_ellipsoid(ax1, lx, ly, lz, 45, col45, 0.7);
plot_ellipsoid(ax1, lx, ly, lz, 90, col90, 0.7);

% bounding box for cuboid - full wireframe
plot_cuboid_box(ax1, 0.6, 0.25);

xlabel(ax1,'x (m)'); ylabel(ax1,'y (m)'); zlabel(ax1,'z (m)');
title(ax1,'(a)','FontWeight','bold');
% improved view - more 3D, better depth perception
view(ax1, 38, 28);
axis(ax1,'equal'); grid(ax1,'on'); box(ax1,'on');
set(ax1,'FontName','Times New Roman','LineWidth',0.8,'GridLineStyle',':','GridAlpha',0.4);
camlight(ax1,'right'); camlight(ax1,'headlight');
lighting(ax1,'gouraud');
% legend - positioned outside overlap
text(ax1, 0.75, -0.35, 0.10, '\theta = 0°','Color',col0,'FontSize',9,'FontWeight','bold');
text(ax1, 0.75, -0.35, 0.06, '\theta = 45°','Color',col45,'FontSize',9,'FontWeight','bold');
text(ax1, 0.75, -0.35, 0.02, '\theta = 90°','Color',col90,'FontSize',9,'FontWeight','bold');

% Panel (b) - u(theta)/u(0)
ax2 = nexttile;
plot(ax2, theta_list, u_ratio0, '-o','LineWidth',1.5,'MarkerSize',7,'MarkerFaceColor','w','Color',[0 0.447 0.741]);
xlabel(ax2,'Orientation angle \theta (degrees)'); ylabel(ax2,'u(\theta)/u(0°)');
title(ax2,'(b)','FontWeight','bold'); grid(ax2,'on'); set(ax2,'FontName','Times New Roman');

% Panel (c) - f1/f_iso monotonic
ax3 = nexttile;
plot(ax3, theta_list, f_ratio_iso, '-d','LineWidth',1.5,'MarkerSize',7,'MarkerFaceColor','w','Color',[0.466 0.674 0.188]);
xlabel(ax3,'Orientation angle \theta (degrees)'); ylabel(ax3,'f_1 / f_{iso}');
title(ax3,'(c)','FontWeight','bold'); grid(ax3,'on'); set(ax3,'FontName','Times New Roman');

% Panel (d) - kappa
ax4 = nexttile;
plot(ax4, theta_list, cond_all, '-^','LineWidth',1.5,'MarkerSize',7,'MarkerFaceColor','w','Color',[0.635 0.078 0.184]);
xlabel(ax4,'Orientation angle \theta (degrees)'); ylabel(ax4,'Estimated condition number \kappa');
title(ax4,'(d)','FontWeight','bold'); grid(ax4,'on'); set(ax4,'FontName','Times New Roman');

fprintf('\nFigure ready - export manually\n');

%% Functions
function plot_ellipsoid(ax, lx, ly, lz, theta_deg, col, alpha)
    [X,Y,Z] = ellipsoid(0,0,0,lx,ly,lz,36);
    th = theta_deg*pi/180; c=cos(th); s=sin(th);
    Xr = c*X - s*Y;
    Yr = s*X + c*Y;
    Zr = Z;
    surf(ax, Xr, Yr, Zr, 'FaceColor', col, 'FaceAlpha', alpha, 'EdgeColor','none', 'FaceLighting','gouraud', 'SpecularStrength',0.3);
end

function plot_cuboid_box(ax, Lx, Ly)
    % full wireframe cuboid for reference
    hx = Lx; hy = Ly; hz = 0.08;
    % bottom rectangle
    xb = [-hx hx hx -hx -hx]; yb = [-hy -hy hy hy -hy]; zb = -hz*ones(size(xb));
    plot3(ax, xb, yb, zb, ':','Color',[0.6 0.6 0.6],'LineWidth',0.7);
    % top rectangle
    xt = [-hx hx hx -hx -hx]; yt = [-hy -hy hy hy -hy]; zt = hz*ones(size(xt));
    plot3(ax, xt, yt, zt, ':','Color',[0.6 0.6 0.6],'LineWidth',0.7);
    % vertical edges
    for k=1:4
        plot3(ax, [xb(k) xt(k)], [yb(k) yt(k)], [zb(k) zt(k)], ':','Color',[0.6 0.6 0.6],'LineWidth',0.7);
    end
end
