...
%  Orientation study for 3D strain-gradient elasticity.
%  Sweeps over theta (orientation angle, degrees, rotation about z-axis)
%  with fixed anisotropic lengths lx=0.20, ly=lz=0.02 m
...
u_avg_all   = zeros(1, nTheta);
f1_all      = zeros(1, nTheta);
condest_all = zeros(1, nTheta);

fprintf('--- Orientation study: lx=%.2f, ly=lz=%.2f ---\n', lx, ly);
for jt = 1:nTheta
    theta_deg = theta_list(jt);
    fprintf('--- theta=%d° ---', theta_deg);
    AAT_rot = build_rotated_AAT(lx, ly, lz, theta_deg);
    [u_avg_all(jt), ~, f1_all(jt), condest_all(jt)] = ...
    ...
    fprintf('  u=%.4f um, f1=%.2f Hz, kappa=%.3e\n', ...
        u_avg_all(jt)*1e6, f1_all(jt), condest_all(jt));
end

l_iso = (lx*ly*lz)^(1/3);
AAT_iso = diag([l_iso^2, l_iso^2, l_iso^2]);
[u_iso, ~, f_iso, ~] = solve_3d_sg_rotated(Lx, Ly, Lz, E, nu, rho, nelx, nely, nelz, ...
                                          AAT_iso, traction_right, body_force, bc_case);
fprintf('\nIsotropic reference l_iso=%.4f: u=%.4f um, f=%.2f Hz\n', l_iso, u_iso*1e6, f_iso);

fprintf('\n===== ORIENTATION SUMMARY =====\n');
fprintf('%8s %12s %12s %12s %12s\n', 'theta', 'u(um)', 'u/u0', 'f1(Hz)', 'condest');
for jt=1:nTheta
    fprintf('%7d° %12.4f %12.6f %12.2f %12.4e\n', theta_list(jt), u_avg_all(jt)*1e6, u_avg_all(jt)/u_avg_all(1), f1_all(jt), condest_all(jt));
end

% Figure
fig = figure('Color','white','Units','centimeters','Position',[2 2 18 14]);
tl = tiledlayout(2,2,'TileSpacing','compact','Padding','compact');
ax1=nexttile; plot(ax1, theta_list, u_avg_all/u_avg_all(1), 'o-', 'LineWidth',1.5); xlabel(ax1,'\theta (°)'); ylabel(ax1,'u/u_0'); title(ax1,'(a) Static'); grid(ax1,'on'); set(ax1,'FontName','Times New Roman');
ax2=nexttile; plot(ax2, theta_list, f1_all, 's-', 'LineWidth',1.5); xlabel(ax2,'\theta (°)'); ylabel(ax2,'f_1 (Hz)'); title(ax2,'(b) Frequency'); grid(ax2,'on'); set(ax2,'FontName','Times New Roman');
ax3=nexttile; plot(ax3, theta_list, f1_all/f_iso, '^-', 'LineWidth',1.5); xlabel(ax3,'\theta (°)'); ylabel(ax3,'f_1/f_{iso}'); title(ax3,'(c) Normalized freq'); grid(ax3,'on'); set(ax3,'FontName','Times New Roman');
ax4=nexttile; plot(ax4, theta_list, condest_all, 'd-', 'LineWidth',1.5); xlabel(ax4,'\theta (°)'); ylabel(ax4,'\kappa'); title(ax4,'(d) Condition'); grid(ax4,'on'); set(ax4,'FontName','Times New Roman');
...
function [avg_ux_right, max_umag, freq_first, cond_est] = ...
    ...
    for ez=1:nelz
        for ey=1:nely
            for ex=1:nelx
                e=e+1;
                conn(e,:) = [node_id(ex,ey,ez), node_id(ex+1,ey,ez), node_id(ex+1,ey+1,ez), node_id(ex,ey+1,ez), node_id(ex,ey,ez+1), node_id(ex+1,ey,ez+1), node_id(ex+1,ey+1,ez+1), node_id(ex,ey+1,ez+1)];
            end
        end
    end
    nel = size(conn,1);
    K = sparse(ndof,ndof); M = sparse(ndof,ndof); F = zeros(ndof,1);
    [gp, gw] = gauss_1d_01(3);
    m11 = AAT_rot(1,1);  m22 = AAT_rot(2,2);  m33 = AAT_rot(3,3);
    m12 = AAT_rot(1,2);  m13 = AAT_rot(1,3);  m23 = AAT_rot(2,3);
    for e = 1:nel
        nodes = conn(e,:); xy = coords(nodes,:);
        hx = xy(2,1)-xy(1,1); hy = xy(4,2)-xy(1,2); hz = xy(5,3)-xy(1,3);
        detJ = hx*hy*hz;
        ...
        for ig = 1:length(gp)
            ...
            for jg = 1:length(gp)
                ...
                for kg = 1:length(gp)
                    ...
                    Kcl = B.'*C*B;
                    Kg = (1/10) * ( m11*(Bx.'*C*Bx) + m22*(By.'*C*By) + m33*(Bz.'*C*Bz) + m12*(Bx.'*C*By + By.'*C*Bx) + m13*(Bx.'*C*Bz + Bz.'*C*Bx) + m23*(By.'*C*Bz + Bz.'*C*By) );
                    Ke = Ke + (Kcl + Kg) * wgt;
                    Me = Me + rho*(Nv.'*Nv) * wgt;
    ...
    for ez = 1:nelz
        for ey = 1:nely
            ...
            nodes = conn(e_right,:); xy = coords(nodes,:);
            hx = xy(2,1)-xy(1,1); hy = xy(4,2)-xy(1,2); hz = xy(5,3)-xy(1,3);
            edofs = element_dofs(nodes,ndof_per_node);
            Fe = zeros(192,1); s = 1.0;
            for jg = 1:length(gp)
                t = gp(jg); wt = gw(jg);
                for kg = 1:length(gp)
                    r = gp(kg); wr = gw(kg);
...
function edofs = element_dofs(nodes,ndof_per_node)
    edofs = zeros(1,numel(nodes)*ndof_per_node); c=0;
    for a=1:numel(nodes)
        n=nodes(a);
...
function [H0,H1,H0x,H1x,H0xx,H1xx] = hermite_1d_two_nodes(s,L)
    h1 = 1 - 3*s^2 + 2*s^3; h2 = L*(s - 2*s^2 + s^3); h3 = 3*s^2 - 2*s^3; h4 = L*(-s^2 + s^3);
    dh1 = -6*s + 6*s^2; dh2 = L*(1 - 4*s + 3*s^2); dh3 = 6*s - 6*s^2; dh4 = L*(-2*s + 3*s^2);
    d2h1 = -6 + 12*s; d2h2 = L*(-4 + 6*s); d2h3 = 6 - 12*s; d2h4 = L*(-2 + 6*s);
    H0 = [h1, h3]; H1 = [h2, h4]; H0x = (1/L)*[dh1, dh3]; H1x = (1/L)*[dh2, dh4]; H0xx = (1/L^2)*[d2h1, d2h3]; H1xx = (1/L^2)*[d2h2, d2h4];
end
function [N,Nx,Ny,Nz,Nxx,Nyy,Nzz,Nxy,Nxz,Nyz] = hermite_shape_3d(s,t,r,hx,hy,hz)
    ...
    xs = [1 2 2 1 1 2 2 1]; ys = [1 1 2 2 1 1 2 2]; zs = [1 1 1 1 2 2 2 2];
...
function [H,Hd,Hdd] = pickH(flag,idx,H0,H1,H0d,H1d,H0dd,H1dd)
    if flag == 0; H=H0(idx); Hd=H0d(idx); Hdd=H0dd(idx); else; H=H1(idx); Hd=H1d(idx); Hdd=H1dd(idx); end
end
function [B,Bx,By,Bz,Nv] = make_B_matrices_3d(N,Nx,Ny,Nz,Nxx,Nyy,Nzz,Nxy,Nxz,Nyz)
    ...
    for a=1:8
        ...
        B(1,up)=Nx(sp); B(2,vp)=Ny(sp); B(3,wp)=Nz(sp); B(4,vp)=Nz(sp); B(4,wp)=Ny(sp); B(5,up)=Nz(sp); B(5,wp)=Nx(sp); B(6,up)=Ny(sp); B(6,vp)=Nx(sp);
        Bx(1,up)=Nxx(sp); Bx(2,vp)=Nxy(sp); Bx(3,wp)=Nxz(sp); Bx(4,vp)=Nxz(sp); Bx(4,wp)=Nxy(sp); Bx(5,up)=Nxz(sp); Bx(5,wp)=Nxx(sp); Bx(6,up)=Nxy(sp); Bx(6,vp)=Nxx(sp);
        By(1,up)=Nxy(sp); By(2,vp)=Nyy(sp); By(3,wp)=Nyz(sp); By(4,vp)=Nyz(sp); By(4,wp)=Nyy(sp); By(5,up)=Nyz(sp); By(5,wp)=Nxy(sp); By(6,up)=Nyy(sp); By(6,vp)=Nxy(sp);
        Bz(1,up)=Nxz(sp); Bz(2,vp)=Nyz(sp); Bz(3,wp)=Nzz(sp); Bz(4,vp)=Nzz(sp); Bz(4,wp)=Nyz(sp); Bz(5,up)=Nzz(sp); Bz(5,wp)=Nxz(sp); Bz(6,up)=Nyz(sp); Bz(6,vp)=Nxz(sp);
    end
end
