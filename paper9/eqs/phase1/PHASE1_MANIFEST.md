...
- **M8-a-1 (new, recorded)** periodic-cell corner-term behaviour: `sum_corners = (tau_i12 +
  ...
  for M15.
- **M15-a (NEW, 2026-09-22 — flagged by M9, NOT repaired, ruling required)**: blueprint Sec 4.5 /
  eq. (68) "prove `Kbar(k) = Kbar(-k)^H`" and the mandatory Sec 4 pitfalls test
  `||Kbar(k) - Kbar(-k)^H||/||Kbar|| < 1e-12` cannot be established in their literal form from the
  locked formulation. In the tying route prescribed by Sec 4.4 one gets **always** `Kbar^H = Kbar`
  and `Kbar(-k) = conj(Kbar(k))`, hence `Kbar(-k)^H = Kbar(k)^T`, so (68) is exactly equivalent to
  "`Kbar` is real (real symmetric)" — an extra condition that real symmetry does not supply. Two
  exact counterexamples and the route diagnosis (the claim is true only for the *complex-symmetric*
  envelope route, whose matrix is not Hermitian) are in `DERIVATION_M09.md` §M9.8, checks [X1]-[X3].
  Separately, a WRONG-SIGN (unimodular) phase on the derivative DOFs keeps `Kbar` Hermitian and keeps
  `Kbar(-k) = conj(Kbar(k))`, so the listed test does not detect the error it targets ([X4]);
  detection needs an independent reference (planned P2.1/P2.2 analytic limits or the Phase-3 anchors).
  No blueprint edit, no equation changed, no test replaced. **Status TO BE VERIFIED — the user's
  ruling is required before M15.**
