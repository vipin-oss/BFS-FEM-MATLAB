...
\subsection{Ellipsoidal Nonlocal Averaging Domain and Length Tensor}
In standard isotropic gradient elasticity, the microstructural influence is encapsulated by a single scalar length scale $l$. To model materials with oriented or directional microstructures, we consider a representative nonlocal averaging neighborhood $\mathcal{E}$ defined as an ellipse in two dimensions with principal semi-axes $l_1$ and $l_2$:
\begin{equation}
...
\subsection{Orientation and Passive Tensor Rotation}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.88\textwidth]{../figures/out/fig01_ellipsoid_tensor.pdf}
\caption{(a) Schematic of the microstructural averaging ellipse at orientation $\theta = 45^\circ$ for aspect ratios $\mathrm{AR} \in \{1, 3, 5, 10\}$ under volume-equivalent scaling ($l_{\mathrm{iso}} = 0.20\,\mathrm{m}$). (b) Evolution of second-moment length tensor components $L_{11}, L_{22}, L_{12}$ as functions of orientation angle $\theta$ for $\mathrm{AR} = 5$, verifying negative off-diagonal coupling $L_{12}(45^\circ) = -0.096\,\mathrm{m}^2$ under locked passive rotation.}
\label{fig:ellipsoid_tensor}
\end{figure}

\subsection{Kinematics and Higher-Order Deformation Gradients}
...
\subsection{Mindlin Form-II Constitutive Relations}
...
\begin{equation}
\tau_{ijk} = L_{kl} C_{ijmn}\eta_{mnl} = L_{kl}\left(\lambda \delta_{ij}\eta_{mml} + 2\mu \eta_{ijl}\right).
\label{eq:double_stress}
\end{equation}
Substituting Eq.~\eqref{eq:double_stress} into Eq.~\eqref{eq:strain_energy_W} demonstrates that $W$ is strictly non-negative and quadratic in both strains and strain gradients.

\subsection{Micro-Inertia and Kinetic Energy Density}
To formulate a causal, physically admissible dynamic theory, the kinetic energy density $T$ is extended to include gradient inertia (micro-inertia):
\begin{equation}
...
\end{equation}
The Laplacian term $-\rho \ell_{\mathrm{i}}^2 \ddot u_{i,jj}$ on the right-hand side represents micro-inertial resistance to localized velocity gradients. As shown in Section~\ref{sec:steering} and Appendix~\ref{app:asymptotics}, this dynamic term regularizes high-frequency dispersion and prevents phase velocity divergence.

\subsection{Limit Ladder and Reductions}
...
\begin{enumerate}[label=(\roman*)]
  ...
  \item \textbf{Gradient Elasticity without Micro-Inertia ($\ell_{\mathrm{i}} = 0$):} Higher-order stiffness double stresses persist while dynamic regularisation is removed, producing an unbounded phase velocity branch ($v_p \propto k \to \infty$).
