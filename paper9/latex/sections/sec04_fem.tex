...
\end{equation}
With 4 vertices and 2 displacement components, each BFS element contains $4 \times 2 \times 4 = 32$ degrees of freedom. Figure~\ref{fig:bfs_dof} illustrates the nodal DOF arrangement and the complex phase mapping on periodic boundaries.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.88\textwidth]{../figures/out/fig03_bfs_dof_bloch.pdf}
...
\begin{equation}
\bmK^g(\theta, \mathrm{AR}) = \sum_{k=1}^2 \sum_{l=1}^2 L_{kl}(\theta) \int_{\Omega_e} (\bmB^g_k)^\mathsf{T} \bmC \bmB^g_l \,\dif\Omega.
\label{eq:Kg_int}
\end{equation}
Because the shape functions are bicubic polynomials, the integrands in $\bmK^c$ contain products of second derivatives (quartic polynomials), while those in $\bmK^g$ contain products of third derivatives (quadratic polynomials). A standard $4 \times 4$ Gauss--Legendre quadrature integrates all terms exactly without aliasing or rank deficiency.

\subsection{Element Mass and Micro-Inertia Matrices}
...
\subsection{Bloch-Periodic Master--Slave Elimination}
...
\subsection{Reduced Hermitian Generalized Eigenproblem}
...
\subsection{Modal Assurance Criterion and Branch Tracking}
